    ...
    def parse_postfix(self) -> ASTNode:
        ...
        while self.current_token().type in (TokenType.DOT, TokenType.LBRACKET):
            if self.current_token().type == TokenType.DOT:
                ...
                # Check if this is a slice (starts with colon)
                if self.current_token().type == TokenType.COLON:
                    ...
                    self.expect(TokenType.RBRACKET)
                    left = SliceAccess(left, start, end, step)
                else:
                    # parse first expression and check for colon for start:end:step
